...
(function () {
  ...
  function listenQuotes(uid) {
    console.log('[DASH][quotes] starting listener for uid =', uid);
    return fbDB.collection('quotes')
      ...
      .onSnapshot(function (snap) {
        ...
        console.log('[DASH][quotes] uid =', uid, '| result count =', quotes.length,
          '| first doc =', quotes[0] || null);
  ...
  function listenOrders(uid) {
    console.log('[DASH][orders] starting listener for uid =', uid);
  ...
  function listenNotifications(uid) {
    console.log('[DASH][notifications] starting listener for uid =', uid);
    return fbDB.collection('notifications')
      ...
      .onSnapshot(function (snap) {
        ...
        console.log('[DASH][notifications] uid =', uid, '| result count =', notifs.length,
          '| first doc =', notifs[0] || null);
  ...
  if (qForm) {
    qForm.addEventListener('submit', function (e) {
      ...
      var data = {
        pickup:       qForm.querySelector('[name=pickup]') ? qForm.querySelector('[name=pickup]').value.trim() : '',
        delivery:     qForm.querySelector('[name=delivery]') ? qForm.querySelector('[name=delivery]').value.trim() : '',
        materialType: qForm.querySelector('[name=materialType]') ? qForm.querySelector('[name=materialType]').value.trim() : '',
        weight:       qForm.querySelector('[name=weight]') ? qForm.querySelector('[name=weight]').value.trim() : '',
        packages:     qForm.querySelector('[name=packages]') ? qForm.querySelector('[name=packages]').value.trim() : '',
        pickupDate:   qForm.querySelector('[name=pickupDate]') ? qForm.querySelector('[name=pickupDate]').value : '',
        notes:        qForm.querySelector('[name=notes]') ? qForm.querySelector('[name=notes]').value.trim() : ''
      };
      ...
      EGC.nextQuoteId().then(function (quoteId) {
        var quoteData = Object.assign({}, data, {
          ...
          companyName:   (qForm.querySelector('[name=companyName]') ? qForm.querySelector('[name=companyName]').value.trim() : '') || null,
  ...
  function custInvoiceCardHTML(inv) {
    ...
    return (
      '<div class="cinv-card">' +
        ...
        '</div>' +
        '<div class="cinv-actions">' +
          '<button class="btn-a btn-sm" onclick="CUST.viewInvoice(\'' + EGC.esc(inv._docId) + '\')">View Invoice</button>' +
          '<button class="btn-ghost btn-sm" onclick="CUST.downloadInvoice(\'' + EGC.esc(inv._docId) + '\')">Download / Print PDF</button>' +
        '</div>' +
      '</div>'
    );
  }
  ...
  window.CUST.viewInvoice = function (docId) {
    ...
    if (inv) INV.openInvoiceWindow(inv, false).then(function (ok) { if (!ok) alert('Please allow pop-ups to view your invoice.'); });
  };
  window.CUST.downloadInvoice = function (docId) {
    var inv = findCustInv(docId);
    if (inv) INV.openInvoiceWindow(inv, true).then(function (ok) { if (!ok) alert('Please allow pop-ups to download your invoice.'); });
  };
