...
(function () {
  ...
  var DEFAULT_CONFIG = {
    ...
    ifsc:          'YESB0000679',
    /* Replaceable signature. A transparent PNG data-URI or hosted URL.
       Empty string => a styled placeholder line is rendered instead. */
    signatureUrl:  '',
  ...
  function orbitLogoSVG(size) {
    size = size || 54;
    return '' +
    '<svg width="' + size + '" height="' + (size * 0.7) + '" viewBox="0 0 130 90" fill="none" xmlns="http://www.w3.org/2000/svg">' +
      '<path d="M58 20 C38 20 22 34 22 52 C22 66 33 76 48 76 C70 76 96 60 108 36" stroke="#f26522" stroke-width="13" stroke-linecap="round" fill="none"/>' +
      '<circle cx="100" cy="20" r="11" fill="#1a1a1a"/>' +
    '</svg>';
  }
...
/* watermark */
'.watermark{position:absolute;top:46%;left:50%;transform:translate(-50%,-50%) rotate(-6deg);font-size:175px;font-weight:900;color:rgba(242,101,34,.07);letter-spacing:-6px;white-space:nowrap;pointer-events:none;z-index:0;font-style:italic;font-family:Georgia,serif;}' +
'.wm2{position:absolute;top:52%;left:50%;transform:translate(-50%,-50%);font-size:66px;font-weight:800;color:rgba(120,130,145,.07);letter-spacing:18px;pointer-events:none;z-index:0;}' +
'.layer{position:relative;z-index:1;}' +
/* header */
'.tax-badge{position:absolute;top:0;right:0;background:#1a1a1a;color:#fff;font-weight:800;font-size:15px;letter-spacing:1px;padding:9px 26px;border-radius:16px;}' +
'.head{display:flex;gap:18px;align-items:flex-start;}' +
'.logo-wrap{flex:none;text-align:center;}' +
...
'.brand-mid{flex:1;text-align:center;padding-top:6px;}' +
...
'.brand-tag{font-size:13px;color:#f26522;font-style:italic;margin-top:8px;position:relative;display:inline-block;padding:0 26px;}' +
...
'.contact{flex:none;width:240px;font-size:10.5px;color:#333;padding-top:30px;}' +
'.contact div{display:flex;gap:7px;margin-bottom:5px;align-items:flex-start;}' +
'.contact .ico{color:#f26522;font-weight:bold;flex:none;width:13px;}' +
/* gst bar */
...
'.left3{flex:1.25;}' +
'.hsn{font-size:11.5px;margin-bottom:26px;}.hsn .ul{display:inline-block;border-bottom:1px solid #bbb;min-width:230px;height:14px;margin-left:4px;}' +
...
'</style></head><body>' +
'<div class="page">' +
  '<div class="watermark">Orbit</div>' +
  '<div class="wm2">LOGISTICS</div>' +
  (status !== 'pending' ? '<div class="statusflag sf-' + status + '">' + esc(window.INV.paymentLabel(status)) + '</div>' : '') +
  '<div class="layer">' +
    '<div class="tax-badge">TAX INVOICE</div>' +
    /* header */
    '<div class="head">' +
      '<div class="logo-wrap">' + orbitLogoSVG(58) +
        '<div class="logo-name">Orbit</div><div class="logo-sub">LOGISTICS</div>' +
      '</div>' +
      '<div class="brand-mid">' +
    ...
    '<div class="row3">' +
      '<div class="left3">' +
        ...
        '<div class="hsn">Remarks : <span class="ul"></span></div>' +
