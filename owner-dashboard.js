...
(function () {
  ...
  window.OWN.approve = function (qid) {
    ...
    Promise.all([EGC.nextOrderId(), EGC.nextInvoiceId(), INV.nextLrNumber(), LR.nextDocketNumber()]).then(function (ids) {
      ...
      var orderData = {
        ...
        shipmentType:  q.shipmentType || 'commercial',
        pickup:        q.pickup,
        delivery:      q.delivery,
      ...
      /* Auto-build the Lorry Receipt — same Order, second document.
         Reuses the invoice record so freight/customer stay consistent. */
      var lrRecord = LR.buildRecord({
        lrNumber:     lrNumber,
    ...
    return (
      '<div class="inv-card">' +
        ...
        '<div class="inv-card-actions">' +
          ...
          '<button class="btn-a btn-sm" style="margin-left:auto;" onclick="OWN.toggleInvoiceEdit(\'' + id + '\')" id="invedit-btn-' + id + '">Edit Invoice</button>' +
        '</div>' +

        /* ── EDIT PANEL (hidden until toggled) ── */
        '<div class="inv-edit-panel" id="invedit-' + id + '" style="display:none;">' +
          '<div class="inv-edit-title">Edit invoice details</div>' +
          '<div class="inv-edit-grid">' +
            '<label>Vehicle Number<input type="text" id="ed-vehicle-' + id + '" value="' + EGC.esc(inv.vehicleNumber || '') + '" placeholder="e.g. MH-12WX-4965"></label>' +
            '<label>LR Number<input type="text" id="ed-lr-' + id + '" value="' + EGC.esc(inv.lrNumber || '') + '"></label>' +
            '<label>Halting Charges (\u20B9)<input type="number" min="0" id="ed-halting-' + id + '" value="' + numv(inv.haltingCharges) + '"></label>' +
            '<label>Extra Charges (\u20B9)<input type="number" min="0" id="ed-extra-' + id + '" value="' + numv(inv.extraCharges) + '"></label>' +
            '<label>Advance Received (\u20B9)<input type="number" min="0" id="ed-advance-' + id + '" value="' + numv(inv.advanceReceived) + '"></label>' +
            '<label>Discount (\u20B9)<input type="number" min="0" id="ed-discount-' + id + '" value="' + numv(inv.discount) + '"></label>' +
            '<label>Due Date<input type="date" id="ed-due-' + id + '" value="' + dueVal + '"></label>' +
            '<label>Payment Status<select id="ed-status-' + id + '">' + statusSel + '</select></label>' +
            '<label class="inv-edit-wide">Remarks<input type="text" id="ed-remarks-' + id + '" value="' + EGC.esc(inv.remarks || '') + '" placeholder="Optional note shown on the invoice"></label>' +
          '</div>' +
          '<div class="inv-edit-actions">' +
            ...
            '<span class="inv-edit-hint">Saving updates the preview, PDF, and both dashboards instantly.</span>' +
          '</div>' +
        '</div>' +
  ...
  window.OWN.toggleInvoiceEdit = function (docId) {
    ...
    if (btn) btn.textContent = open ? 'Edit Invoice' : 'Close Editor';
  };

  /* Save all owner-editable fields. Firestore update auto-syncs the
     preview, PDF, customer view and owner view via existing onSnapshot. */
  window.OWN.saveInvoiceEdits = function (docId) {
    ...
    var vehicleNumber = val('vehicle').trim();
    var lrNumber      = val('lr').trim();
    var remarks       = val('remarks').trim();
    var halting       = numv('halting');
    var extra         = numv('extra');
    var advance       = numv('advance');
    var discount      = numv('discount');
    var newStatus     = val('status') || inv.paymentStatus;
    var dueStr        = val('due');

    /* Recompute totals with the edited charge fields */
    var merged = Object.assign({}, inv, {
      haltingCharges:  halting,
      extraCharges:    extra,
      advanceReceived: advance,
      discount:        discount
    });
    var t = INV.computeTotals(merged);

    /* Keep status coherent: if fully covered, mark paid */
    var received = INV.toNum(inv.receivedAmount);
    if ((received + advance) >= t.invoiceValue && t.invoiceValue > 0 && newStatus !== 'paid') {
      /* don't silently override an explicit owner choice other than pending/partial */
      if (newStatus === 'pending' || newStatus === 'partial') newStatus = 'paid';
    }
    ...
    var update = {
      ...
      invoiceAmount:     t.invoiceValue,
      outstandingAmount: t.outstanding,
      paymentDate:       (newStatus === 'paid') ? firebase.firestore.FieldValue.serverTimestamp() : inv.paymentDate || null,
      updatedAt:         firebase.firestore.FieldValue.serverTimestamp()
    };
    ...
    /* If LR number changed, keep the order's lrNumber in sync too */
    var batch = fbDB.batch();
    batch.update(fbDB.collection('invoices').doc(inv.invoiceId), update);
    if (lrNumber && lrNumber !== inv.lrNumber && inv.orderId) {
      batch.update(fbDB.collection('orders').doc(inv.orderId), {
        lrNumber: lrNumber, updatedAt: firebase.firestore.FieldValue.serverTimestamp()
      });
    }

    batch.commit().then(function () {
      if (msg) { msg.className = 'fst ok'; msg.textContent = '\u2713 Invoice updated. Preview, PDF and customer view now reflect the changes.'; }
      toast(true, inv.invoiceNumber + ' updated');

      EGC.createNotification(inv.customerUid, 'invoice_updated',
        'Your invoice ' + inv.invoiceNumber + ' was updated. Net payable: \u20B9' + INV.fmtMoney(t.netPayable) + ' \u00B7 ' + INV.paymentLabel(newStatus) + '.',
        { invoiceId: inv.invoiceId, orderId: inv.orderId });
      EGC.logActivity(inv.customerUid, 'invoice_updated',
        'Invoice ' + inv.invoiceNumber + ' updated (status: ' + INV.paymentLabel(newStatus) + ')',
        { invoiceId: inv.invoiceId });
      EGC.logAudit('payment_recorded',
        'Invoice ' + inv.invoiceNumber + ' edited \u2014 status ' + INV.paymentLabel(newStatus) +
          ', net payable \u20B9' + INV.fmtMoney(t.netPayable) + '.',
      ...
      /* close editor; the snapshot listener will re-render the card with fresh data */
      var panel = $('#invedit-' + docId);
      var btn   = $('#invedit-btn-' + docId);
      if (panel) panel.style.display = 'none';
      if (btn) btn.textContent = 'Edit Invoice';
    }).catch(function (err) {
      if (msg) { msg.className = 'fst er'; msg.textContent = err.message || 'Could not save invoice changes.'; }
    });
  };
  ...
  function ownerLrCardHTML(lr) {
    var t  = LR.computeTotals(lr);
    var id = EGC.esc(lr._docId);
      ...
      '<div class="lr-card">' +
        '<div class="lr-card-top">' +
          '<div><div class="lr-id">' + EGC.esc(lr.lrNumber) + '</div>' +
            '<div class="lr-sub">Docket ' + EGC.esc(lr.docketNumber || '\u2014') + ' \u00B7 ' + EGC.esc(lr.orderId || '') + ' \u00B7 ' + EGC.fmtWhen(lr.createdAt) + '</div></div>' +
        ...
        '<div class="lr-card-actions doc-dl-group">' +
          ...
          '<button class="btn-a btn-sm" style="margin-left:auto;" onclick="OWN.toggleLrEdit(\'' + id + '\')" id="lredit-btn-' + id + '">Edit LR</button>' +
        '</div>' +

        /* ── EDIT PANEL ── */
        '<div class="lr-edit-panel" id="lredit-' + id + '" style="display:none;">' +
          '<div class="lr-edit-title">Edit Lorry Receipt</div>' +
          '<div class="lr-edit-grid">' +
            ...
            '<label>LR Number<input type="text" id="lr-lrno-' + id + '" value="' + EGC.esc(lr.lrNumber || '') + '"></label>' +
            '<label>Actual Weight (KG)<input type="text" id="lr-aweight-' + id + '" value="' + EGC.esc(lr.actualWeight || '') + '"></label>' +
            '<label>Charged Weight (KG)<input type="text" id="lr-cweight-' + id + '" value="' + EGC.esc(lr.chargedWeight || '') + '"></label>' +
          ...
          '<div class="lr-edit-actions">' +
            ...
            '<span class="lr-edit-hint">Saving updates the LR preview, PDFs and both dashboards instantly.</span>' +
          '</div>' +
        '</div>' +
  ...
  window.OWN.viewLR = function (docId) {
    var lr = findLr(docId);
    if (lr) LR.openLrWindow(lr, false).then(function (ok) { if (!ok) toast(false, 'Allow pop-ups to view the Lorry Receipt.'); });
  };
  window.OWN.printLR = function (docId) {
    var lr = findLr(docId);
    if (lr) LR.openLrWindow(lr, true).then(function (ok) { if (!ok) toast(false, 'Allow pop-ups to print the Lorry Receipt.'); });
  };
  window.OWN.printCombined = function (docId) {
    ...
    LR.openCombinedWindow(inv, lr, true).then(function (ok) { if (!ok) toast(false, 'Allow pop-ups to print the combined PDF.'); });
  };
  ...
  window.OWN.toggleLrEdit = function (docId) {
    ...
    var open = panel.style.display !== 'none';
    panel.style.display = open ? 'none' : 'block';
    if (btn) btn.textContent = open ? 'Edit LR' : 'Close Editor';
  };
  ...
  window.OWN.saveLrEdits = function (docId) {
    ...
    var newLrNo = val('lrno').trim() || lr.lrNumber;
    ...
    var update = {
      ...
      packageCount:        val('pkg').trim(),
      packingMethod:       val('packing').trim(),
      dispatchMode:        val('dispatch').trim(),
      materialDescription: val('material').trim(),
      ...
      sgstRate:            numv('sgst'),
      cgstRate:            numv('cgst'),
      paymentStatus:       val('status') || lr.paymentStatus,
    ...
    /* Refresh the accounting-ready snapshot total */
    var merged = Object.assign({}, lr, update);
    update.grandTotalAmount = LR.computeTotals(merged).grandTotal;

    /* The LR doc id is the LR number. If the owner renames the LR number,
       keep the existing doc (id stays stable) but store the new display
       number, and sync order/invoice so all three stay aligned. */
    var batch = fbDB.batch();
    batch.update(fbDB.collection('lorryReceipts').doc(lr._docId), update);
    if (newLrNo !== lr.lrNumber) {
      if (lr.orderId)   batch.update(fbDB.collection('orders').doc(lr.orderId), { lrNumber: newLrNo, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
      if (lr.invoiceId) batch.update(fbDB.collection('invoices').doc(lr.invoiceId), { lrNumber: newLrNo, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
    }

    batch.commit().then(function () {
      if (msg) { msg.className = 'fst ok'; msg.textContent = '\u2713 Lorry Receipt updated. Preview, PDFs and customer view now reflect the changes.'; }
      toast(true, newLrNo + ' updated');

      EGC.createNotification(lr.customerUid, 'lr_updated',
        'Your Lorry Receipt ' + newLrNo + ' was updated with the latest transport details.',
        { lrNumber: newLrNo, orderId: lr.orderId });
      EGC.logAudit('lr_generated',
        'Lorry Receipt ' + newLrNo + ' edited (vehicle ' + (update.vehicleNumber || '\u2014') + ', driver ' + (update.driverName || '\u2014') + ').',
        { targetType: 'lorryReceipt', targetId: lr._docId, orderId: lr.orderId, quoteId: lr.quoteId,
          previousValue: lr.vehicleNumber || null, newValue: update.vehicleNumber || null });

      var panel = $('#lredit-' + docId);
      var btn   = $('#lredit-btn-' + docId);
      if (panel) panel.style.display = 'none';
      if (btn) btn.textContent = 'Edit LR';
    }).catch(function (err) {
      if (msg) { msg.className = 'fst er'; msg.textContent = err.message || 'Could not save LR changes.'; }
    });
  };
