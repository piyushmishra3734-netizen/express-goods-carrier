...
(function () {
  ...
  /* ============================================================
     AUDIT LOG — business tracking, accountability & proof
     Every important action (quote/order/notification/invoice/
     payment lifecycle) is recorded here with who did it, when,
     and what changed. Logs are append-only: no update/delete
  ...
  var AUDIT_LABELS = {
    ...
    payment_recorded:    'Payment Recorded' /* reserved for future use */
  };
   ...
   * @param {object} [details]  { targetType, targetId, previousValue, newValue }
   */
  window.EGC.logAudit = function (action, summary, details) {
    ...
    return fbDB.collection('auditLogs').add(entry).catch(function (err) {
      console.error('[EGC] audit log write failed:', err.message);
    });
  };
