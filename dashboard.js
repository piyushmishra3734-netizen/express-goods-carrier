...
(function () {
  ...
  window.CUST.downloadLR = function (docId) {
    ...
    window.LR.openLrWindow(lr, true).then(function (ok) { if (!ok) alert('Please allow pop-ups to download the Lorry Receipt.'); });
  };
  window.CUST.downloadCombined = function (docId) {
    ...
    if (!inv) return;
    if (!lr) { alert('Lorry Receipt is being prepared for this order. Please check back shortly.'); return; }
    window.LR.openCombinedWindow(inv, lr, true).then(function (ok) { if (!ok) alert('Please allow pop-ups to download the combined PDF.'); });
  };
